  ...
  summaryRows.forEach((row) => {
    ...
    if (row.key === 'fixedCosts') {
      costsByCategory.forEach((cat) => {
        const catMonthly = cat.items.reduce((sum, item) => sum + (item.amount || 0), 0);
...
export async function exportBudgetCsv(rows, meta) {
  ...
  const base = meta.fileBaseName || 'flexible-budget';
...
export async function exportBudgetXlsx(rows, meta) {
  ...
  const fileBase = meta.fileBaseName || 'flexible-budget';
