...
import { getHealthMemberBudgetLine, getHealthMemberMonthlyAmount } from './healthInsuranceBudget';
...
function subscriptionLabel(sub, t) {
  ...
  return translated !== transKey ? translated : (sub.name || 'Subscription');
}

function otherCostLabel(cost, t) {
  const transKey = `onboarding.otherCosts.q12.costs.${cost.name}`;
  const translated = t(transKey);
  return translated !== transKey ? translated : (cost.name || 'Other expense');
}

function childFieldLabel(fieldKey) {
  const map = {
    nursery: 'Nursery',
    school: 'School',
...
function collectFixedLineItems(sections, household, t) {
  ...
  if (housing.utilities) {
    buckets.utilities.push(lineItem('fixed-utilities', t('sectionEdit.housing.utilities'), housing.utilities, 'monthly', {
  ...
  if (gt.wasteTax && gt.wasteTaxAmount) {
    buckets.waste_tax.push(lineItem(
      'fixed-waste-tax',
      t('onboarding.housing.q6g.wasteTax'),
      gt.wasteTaxAmount,
      'annual',
      {
  ...
  if (gt.tvLicence && gt.tvLicenceAmount) {
    buckets.tv_licence.push(lineItem(
      'fixed-tv-licence',
      t('onboarding.housing.q6g.tvLicence'),
      gt.tvLicenceAmount,
      'annual',
      {
        editKind: 'housing_govt_tax',
  ...
  if (gt.radioLicence && gt.radioLicenceAmount) {
    buckets.radio_licence.push(lineItem(
      'fixed-radio-licence',
      t('onboarding.housing.q6g.radioLicence'),
    ...
    transport.vehicles.forEach((v, vi) => {
      ...
      if (v.hasInsurance && v.insurancePremium) {
        buckets.transport.push(lineItem(
          ...
          v.insurancePremium,
          v.insuranceFrequency || 'annual',
          {
            editKind: 'transport_vehicle',
            editRef: { vehicleIndex: vi, field: 'insurance' },
            dateType: 'renewal',
            sectionId: 'transport',
            renewalDate: v.insuranceRenewalDate || null,
            source: { vehicle: v },
          },
        ));
      }
  ...
  if (housing.govtTaxes?.customItems) {
    housing.govtTaxes.customItems.forEach((item, idx) => {
      ...
      buckets.custom_taxes.push(lineItem(
        `rec-custom-tax-${idx}`,
        item.label || t('onboarding.housing.q6g.customPlaceholder'),
        item.amount,
...
const EXPENSE_SECTION_LABEL_KEY = {
  housing: 'onboarding.budget.q14.cat.housing',
  transport: 'onboarding.budget.q14.cat.transport',
  health: 'onboarding.budget.q14.cat.health',
  children: 'onboarding.budget.q14.cat.children',
  pets: 'onboarding.budget.q14.cat.pets',
  subscriptions: 'onboarding.budget.q14.cat.subscriptions',
  other: 'onboarding.budget.q14.cat.other',
  debts: 'onboarding.budget.q14.debtPayments',
};
