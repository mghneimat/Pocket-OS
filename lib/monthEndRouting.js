...
export function computeMonthLeftover({ spendingBudget, spentInPeriod }) {
  ...
  return Math.max(0, Math.round(budget - spent));
}
...
export function routeLeftover({ leftover, budget, monthlyFlexible, rolloverBalance }) {
  const amount = Math.max(0, Math.round(leftover));
  ...
  if (strategy === 'capped') {
    const cap = computeRolloverCap(budget, monthlyFlexible);
    const maxRollover = cap != null ? cap : currentRollover + amount;
    const room = Math.max(0, maxRollover - currentRollover);
    const toRollover = Math.min(amount, room);
    const excessToLoose = amount - toRollover;
    return {
      destination: 'rollover',
      amount: toRollover,
      excessToLoose: excessToLoose > 0 ? excessToLoose : undefined,
      newRolloverBalance: currentRollover + toRollover,
    };
  }
...
export function buildMonthEndPreview({
  ...
  return {
    ...
    resetDestination: budget?.rolloverStrategy === 'reset'
      ...
      : null,
    otherGoalNote: budget?.resetOtherGoalNote || null,
    rolloverCap: computeRolloverCap(budget, effectiveMonthlyFlexible),
  };
}
...
export async function processMonthEndIfNeeded({
  ...
  if (!budget.lastClosedPeriod) {
    ...
    await setData('pocketos_budget', budget);
  ...
  if (changed) {
    await setData('pocketos_budget', budget);
    if (income) await setData('pocketos_income', income);
  }
