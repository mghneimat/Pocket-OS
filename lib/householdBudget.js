...
import { toMonthly, totalMonthlyCosts, availableBudget } from './finance';
  ...
  if (transport.hasVehicle && transport.vehicles) {
    transport.vehicles.forEach((v, vi) => {
      ...
      if (v.hasInsurance && v.insurancePremium) {
        transportItems.push({ label: `${prefix}Insurance`, amount: v.insurancePremium, frequency: v.insuranceFrequency || 'annual' });
      }
...
export function loadRecurringCommitments(sections, debts, t) {
  ...
  if (transport.hasVehicle && transport.vehicles) {
    transport.vehicles.forEach((v, vi) => {
      ...
      const freq = v.insuranceFrequency || 'annual';
      const monthlyAmount = toMonthly(parseFloat(v.insurancePremium), freq);
      items.push({
        id: `vehicle-ins-${vi}`,
        kind: 'vehicle_insurance',
        label: prefix,
        amount: parseFloat(v.insurancePremium),
        frequency: freq,
        monthlyAmount,
        annualAmount: monthlyAmount * 12,
...
export async function loadHouseholdFinancials(t) {
  const [
    ...
    getData('pocketos_income'),
    getData('pocketos_debts'),
    getData('pocketos_location'),
    getData('pocketos_household'),
    getData('pocketos_housing'),
    getData('pocketos_transport'),
    getData('pocketos_health'),
    getData('pocketos_children_costs'),
    getData('pocketos_pets'),
    getData('pocketos_subscriptions'),
    getData('pocketos_other_costs'),
    getData('pocketos_budget'),
  ]);
