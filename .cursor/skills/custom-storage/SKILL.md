...
name: custom-storage
description: >-
  Manages PocketOS local persistence via lib/storage.js and lib/schema.js —
...
# PocketOS Custom Storage Layer

PocketOS persists data through two paired modules:
...
3. Update `pocketos_onboarding` when the user advances onboarding
...
2. **Grep existing keys** — `grep pocketos_ lib/` and `app/` before inventing a new key.
3. **One key per screen domain** — e.g. `pocketos_housing` → `Housing` type.
4. **JSON only** — `getData`/`setData` auto `JSON.parse`/`JSON.stringify`; store plain objects/arrays.
5. **Do not use `pocketos_costs`** — legacy/unused; cost data lives in per-section keys.
...
|-------------|-------------------------------|
| `pocketos_onboarding` | `OnboardingState` |
| `pocketos_household` | `Household` |
| `pocketos_location` | `Location` |
| `pocketos_occupation` | `Occupation` |
| `pocketos_income` | `Income` |
| `pocketos_housing` | `Housing` |
| `pocketos_transport` | `Transport` |
| `pocketos_health` | `HealthInsurance` |
| `pocketos_children_costs` | `ChildrenCosts` |
| `pocketos_pets` | `Pets` |
| `pocketos_subscriptions` | `Subscriptions` |
| `pocketos_other_costs` | `OtherCosts` |
| `pocketos_debts` | `Debts` |
| `pocketos_budget` | `Budget` |
| `pocketos_settings` | `Settings` |
| `pocketos_daily_log` | `DailyLog[]` |
| `pocketos_alerts` | `Alert[]` |
| `pocketos_costs` | `Cost[]` — **unused, do not write** |
...
- [ ] setData('pocketos_onboarding', { completed: false, currentStep, percentComplete })
...
### `OnboardingState` (`pocketos_onboarding`)
...
2. Pick `pocketos_<routeName>` matching the screen file.
...
| Settings merge | `const s = await getData('pocketos_settings') \|\| {}; await setData('pocketos_settings', { ...s, language })` |
...
- Using `pocketos_costs` — never written
